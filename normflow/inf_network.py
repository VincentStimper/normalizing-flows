...
#Content from https://github.com/abdulfatir/planar-flow-pytorch
#Start
...
class VAE(nn.Module):
    def __init__(self, flows):
        ...
        self.fce1 = nn.Linear(256, 40)
        self.fce2 = nn.Linear(256, 40)
        self.dec = nn.Sequential(nn.Linear(40, 256), nn.ReLU(True), nn.Linear(256, 512), nn.ReLU(True), nn.Linear(512, 784))
        self.flow = flows
    ...
    def forward(self, x):
        ...
        self.test_params = [torch.mean(-log_prior_zK), torch.mean(log_q0_z0), torch.mean(SLDJ.view(-1))]
