...
class HamiltonianMonteCarlo(Flow):
    ...
    def forward(self, z, return_probabilities=False):
        # Draw momentum
        ...
        probabilities = torch.where(probabilities > 1.0,
            torch.ones_like(probabilities), probabilities)
        ...
        if return_probabilities:
            return z_out, \
                self.target.log_prob(z) - self.target.log_prob(z_out), \
                probabilities
        else:
